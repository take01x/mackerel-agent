...
// Version represents version of mackerel-agent
type Version struct {
	Version  string
	Revision string
}
...
func wrapSpecMeta(meta map[string]interface{}, ver, rev string) map[string]interface{} {
...
func (app *App) UpdateHostSpecs() {
	...
	meta = wrapSpecMeta(meta, app.Version.Version, app.Version.Revision)
...
func Prepare(conf *config.Config, ver *Version) (*App, error) {
	api, err := NewMackerelClient(conf.Apibase, conf.Apikey, ver.Version, ver.Revision, conf.Verbose)
	if err != nil {
...
func RunOnce(conf *config.Config, ver *Version) error {
	graphdefs, hostSpec, metrics, err := runOncePayload(conf, ver)
	if err != nil {
		return err
	}
...
func runOncePayload(conf *config.Config, ver *Version) ([]mackerel.CreateGraphDefsPayload, *mackerel.HostSpec, *agent.MetricsResult, error) {
	hostname, meta, interfaces, customIdentifier, err := collectHostSpecs()
	if err != nil {
		logger.Errorf("While collecting host specs: %s", err)
		return nil, nil, nil, err
	}
	meta = wrapSpecMeta(meta, ver.Version, ver.Revision)
